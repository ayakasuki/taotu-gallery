...
v0.3.1-pre-fix · [MIT License](LICENSE)
...
### v0.3.1-pre-fix（正式版发布前修复）

本次提交用于把 `0.3.1` 的大规模前后台重构整理到可部署状态：补齐数据库迁移，更新初始化说明，确认空库启动可自动建表、迁移、创建首个管理员，并把本轮新增的后台页面、公告通知、备份恢复和站点配置规则写入文档。部分图标仍允许为占位资源，等待正式替换后再进入最终发布。
...
- `conditions.tag_id` 是 v0.3.1-pre-fix 补齐的关键字段，用于把条件标签配置稳定关联到真实 `tags.id`；旧库升级时迁移会回填已有条件标签关联。
...
v0.3.1 起，访客端、普通用户端和管理后台逐步统一到浅色玻璃拟态视觉风格，覆盖默认布局、导航、首页图库、相册、图片详情、上传、API 文档、登录注册、仪表盘和主要后台管理页。图标资产集中放入 `client/public/icons/`，当前仍允许存在占位图标，等待最终图标替换。
