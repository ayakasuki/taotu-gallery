...
| 模块 | 功能 |
|------|------|
| 图库展示 | 瀑布流 / 网格 / 静态 / 轮播四种模式，公共图库 / 我的图库 / 用户图库筛选 |
...
- 注册系统接入 SMTP + Redis：后台网站配置可保存 SMTP 并发送测试邮件，开放注册状态由公开站点配置控制，注册页支持图片人机验证码和可选邮箱验证码。
- 邮件验证码规则改为固定 2 个数字 + 3 个英文并乱序，验证码写入 Redis，2 分钟过期；重发会覆盖旧验证码。
- 登录页新增忘记密码弹窗，使用绑定邮箱验证码 + 二次人机验证码重置密码；前台旧的后端地址设置页和连接检查跳转已移除。
- 仪表盘用户页新增绑定邮箱展示和更改密码卡片；普通用户仪表盘仍按统计、图片管理、相册、用户、API Token、标签设置等子菜单组织。
- 用户私有标签补齐互斥字段和迁移，普通用户只能让自己的私有标签彼此互斥；管理员可管理自己的私有标签与平台标签互斥，API 权限隔离防止跨用户互斥绑定。
- 标签管理支持多选批量操作：管理员后台可批量设为公共或删除；仪表盘可批量删除，管理员自己的仪表盘可批量设为公共。
- 管理员后台标签表格修复操作按钮换行问题，编辑/删除固定在每行最右侧；系统标签不可删除或公共切换。
- 网站配置新增备案号，写入数据库 `site_config`，公开接口返回后在网站页脚居中白色斜体显示。
- 上传成功后在上传区下方展示源地址，支持单项复制和批量复制，继续追加文件不会重复上传已成功图片。
- 自定义路径、站点公网域名、备案号等均以数据库配置为准；嵌入代码和上传返回 URL 使用后台配置的对外域名。
