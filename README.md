...
- 重构普通用户仪表盘，按统计、图片管理、相册、用户、API Token、标签设置等子菜单组织；普通用户入口只保留有权限的功能。
- 图片管理支持图库来源、相册、文件名、标签等组合筛选；管理员后台编辑图片时可同时维护平台标签和图片所属用户的私有标签，且不会误删用户已配置的私有标签。
- 用户上传图片可立即创建并应用自己的私有标签；个人图片管理允许清空私有标签，空选择保存表示移除自己的私有标签。
- 标签系统新增系统分组、未标签/已标签虚拟系统标签；互斥标签支持多个 ID、`.`/`,` 分隔、`u13` 用户标签 ID，并在保存时自动形成互斥组闭包。
- 图库筛选和 API 接口页会灰显互斥标签；后端 API 遇到互斥标签组合会返回 `400 JSON`，不会继续查询空结果。
- 上传成功后在上传区下方展示源地址，支持单项复制和批量复制，继续追加文件不会重复上传已成功图片。
- 条件标签的横图/竖图/正方图互斥判定更严格，接近 `0.9:1.1` 或 `1:1` 的图片归为正方图。
- 自定义路径和站点公网域名均以数据库配置为准；嵌入代码和上传返回 URL 使用后台配置的对外域名。
...
| 用户私有标签 | `user_tags` 表 | 仅创建者可见，ID 以 `u` 前缀区分 |
...
图库和 API 接口页会禁用互斥标签；直接拼接 API URL 时，后端会对非法互斥组合返回 `400` JSON。
