...
v0.3.1-Pre · [MIT License](LICENSE)
...
| 网站外观 | 自定义背景图（支持模糊度调节）、网站 Icon 上传、备案号页脚、统一图标资产与用户端玻璃拟态主题 |
...
### v0.3.1-Pre（用户端 / 访客端基础 UI 迭代）

本次提交用于记录一轮较大的用户端和访客端 UI 迭代，核心目标是先把普通用户和访客能直接感知的页面体验统一起来。管理后台仅做了少量配套适配，整体后台视觉暂未同步重构；部分图标仍使用占位或临时资产，后续会在图标替换和管理后台一起完成后再发布正式版本，因此版本标记为 `Pre`。

- 全局用户端视觉重构：默认布局、导航、页脚、背景、按钮、输入框、卡片、空状态、分页等基础组件统一为更柔和的玻璃拟态风格，并引入统一图标目录与字体资源。
- 首页图库继续保留“网格 / 瀑布流”两种模式，访客默认展示仍读取 `/api/gallery/config`；本轮重点改善了访客端导航、图片卡片、筛选和整体浏览观感。
- 相册页与相册详情页进行了大幅 UI 迭代，优化列表/网格切换、相册卡片、图片选择、封面设置、分页和详情页图片管理体验。
- 上传页重构为新的上传工作台，强化文件上传、URL 上传、API 上传入口、上传进度、成功链接展示和批量复制体验；上传成功文件会从待上传队列移除，继续选择文件为追加。
- 图片详情页重构展示层，优化预览、缩略/中等/完整尺寸入口、嵌入代码、下载/复制、上下图切换和基础信息布局。
- API 文档页重构为更清晰的示例与参数面板，保留 Token 认证、随机图片、相册、标签、嵌入代码等说明，并改善调试与复制体验。
- 登录、注册、忘记密码相关页面更新为统一视觉，继续支持图片验证码、邮箱验证码、开放注册开关、忘记密码弹窗和 Redis 验证码流程。
- 普通用户仪表盘重构为“统计、图片管理、标签设置、账户与安全”四个入口，移除仪表盘内相册入口；相册管理统一回到相册页完成。
- 仪表盘统计页新增用户维度概览接口 `/api/internal/dashboard/overview`，展示图片数量、存储配额、私有标签、相册数量、最近上传、存储圆环和账号摘要。
- 仪表盘图片管理重构为表格型管理面板，支持我的图库来源、相册筛选、文件名搜索、私有标签筛选、批量公开/取消公开/删除、公开状态切换、编辑私有标签、分页和可滚动列表。
- 仪表盘标签设置重构为同屏两块：私有标签管理和人工标签。私有标签支持分页、当前页全选、批量删除、新建/编辑弹窗、可组合/互斥设置；人工标签支持选图、选择已有私有标签或回车创建新标签后执行打标。
- 账户与安全页整合原用户页和 API Token 页，支持头像 hover 上传、账号信息统计卡、旧密码改密、密码显隐切换、成功/失败提示、Token 列表、Token 遮罩、每页 3 条分页和一次性新 Token 展示。
- 后端配套补充用户头像静态资源服务、头像上传接口、Dashboard overview 内部接口，并让当前用户 Token 列表返回可遮罩展示所需的 token 字段。
- 管理后台暂未进行本轮同规格视觉重构，只做了与全局样式、图标、站点配置相关的轻量适配；后续会单独迭代后台后再进入正式版本。
...
v0.3.1-Pre 起，访客端和普通用户端统一使用新的玻璃拟态视觉风格，覆盖默认布局、首页图库、相册、图片详情、上传、API 文档、登录注册和仪表盘。图标资产已集中放入 `client/public/icons/`，当前仍有部分占位图标，正式版会在管理后台 UI 一起迭代后统一替换。
...
> v0.3.1-Pre 主要提升普通用户与访客基础 UI。管理后台功能仍可用，但后台整体视觉暂未按本轮用户端标准重构，计划后续单独迭代。
