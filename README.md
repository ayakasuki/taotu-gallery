...
- 首页图库滚动体验继续修缮：网格和瀑布流仍保持最短列贴合布局，但新图片先生成固定比例的平均色块占位，再由前端队列按缩略图逐张请求和淡入，避免一次性并发请求导致页面卡顿。
- 首页图片卡片由中等图改为优先使用缩略图；图片加载完成后释放对应的可见性监听，网格模式屏外卡片使用浏览器绘制跳过能力，降低大量已加载图片上下滚动时的重复绘制成本。
- 首页加载更多补齐首次硬刷新场景：布局完成后会主动检查底部触发点，避免首次进入页面滚到底部不加载、需要上滚再下滚才触发的问题。
- 网格卡片额外信息层按需渲染，标题、上传者和标签等浮层只在鼠标悬停或聚焦时进入页面结构；同时移除多处模糊滤镜和位移动画，减少多图滚动时的合成压力。
- 普通用户仪表盘概览微调：最近上传改为最多 25 张、5 列 5 行展示；存储使用和账号摘要组合高度与最近上传面板对齐，账号摘要字号与行距同步收口。
