...
30. **普通用户绝不能越权后台和私有媒体** — `/admin` 页面、后台导航、后台数据和所有站点级编辑能力只允许 `role=admin`；普通用户即使用 curl、手改 URL、复用 JWT/session，也不能访问或看到综合配置、条件标签、数据库、备份、云同步、用户管理、站点配置、平台标签等任何后台页面/API/数据。普通用户功能只能放在仪表盘或普通用户专属 API，后端必须同时做角色鉴权和资源所有权校验，不能只靠前端隐藏按钮；`/image/*` 与 `/thumb/*` 这类 hash 媒体直链也必须校验公开状态、公共相册、所有者或管理员，不能把 hash 当成权限。`site_config.registration.privacyMode` 是站点级隐私模式，开启后访客访问图库、相册、API 文档由前端导向登录页，公开图片/相册/嵌入/标签 API 和媒体直链无 JWT/API Token 必须返回 `403`，不能只隐藏页面。
...
65. **移动端重适配后置** — 当前 `0.3.2` 系列以桌面端主要流程稳定为准；移动端主副标题拆分、面板收纳和窄屏重排属于后续 `0.4.x` 迭代，再稳定进入 `1.0.0`。
66. **相册隐藏主页图库只影响首页** — `albums.hide_from_gallery` 是 `0.3.2` 的相册级主页可见性开关；开启后该相册图片必须从首页公共图库、我的图库和管理员用户图库列表/统计里排除，但不能影响相册详情、图片详情、随机图 API、图片公开属性和已有外链访问逻辑。
67. **上传与图片处理迁移到用户组** — `0.3.2` 起最大文件大小、并发上传、周期上传限额、路径命名、文件命名、中等图尺寸、图片质量和允许格式都归 `user_groups` 管理；不要再把这些能力恢复为站点配置里的全局“上传与图片处理”卡片。新用户注册和首个管理员初始化必须自动进入默认用户组。
68. **存储策略只允许管理员配置** — `storage_strategies` 是管理员后台能力，普通用户 API 必须 403；默认本地策略自动创建且不可删除，挂载类型创建后不可修改。策略与用户组是多对多关系，必须通过 `storage_strategy_groups` 读写；`storage_strategies.user_group_id` 仅保留旧数据兼容和首个关联镜像，不要再当唯一关联来源。当前本地策略已真实参与上传路径，COS/OSS/FTP/SFTP 先保留配置入口，未接入上传流时必须明确拒绝而不是静默落回错误位置。
69. **NSFW 状态必须参与公开展示门禁** — `images.nsfw_status=true` 的图片对非管理员公开图库、相册、随机图 API 和 `/image`/`/thumb` 直链都不能公开展示；所有者/管理员的管理视图可以看到记录。审核服务在 `server/services/nsfwService.js`，开启审核的用户组上传图片必须同步审核出明确 `0/1` 结果，服务不可用、缺配置、超时或返回格式无效时必须上传失败并回滚图片记录、原图和缩略图，不允许把跳过/`NULL` 当安全入库；定时任务只补扫 `NULL` 状态和启用后重排队的历史图片。
70. **NSFW 引擎兼容和阈值语义不能回退** — `nsfw.js` 需同时兼容官方 `predictions: [{ className, probability }]` 和 `arnidan/nsfw-api` 扁平小写返回（`porn/sexy/hentai/neutral/drawing`）；保存配置的健康检查图使用 `299x299` JPEG 以匹配 `ghcr.io/arnidan/nsfw-api:latest` 默认 image model。每个类别独立检查，任一受限类别超过阈值即不合规；`100%` 表示该项不参与限制，`0%` 表示只要命中该项就不合规。
71. **NSFW 状态 UI 必须清晰** — 上传结果表必须显示“内容审核”状态；后台图片管理和仪表盘图片管理必须显示“健康内容”列：安全为 `BoolStatusIcon` 绿色圆勾，不健康为粉色圆叉，未开启/未检测为 filled `help-circle` 问号，颜色固定 `#ffb226`，hover 文案说明“? 代表未开启 NSFW 状态的用户组图片”。不要恢复文字“无”或灰色问号。
72. **自定义路径删原图必须显式授权** — `custom_paths.allow_delete` 默认 `false`；删除自定义路径扫描入库图片时，默认只删数据库记录、`image_tags` 和 `data/gallery/.derived` 派生缩略图，不删外部原图。只有后台路径配置开启“管理图片删减”后，图片管理/用户删除等删除操作才允许同步删除该路径原始文件。已配置路径可编辑绑定相册、绑定标签、批量公开和删减开关，但路径字符串本身必须锁定不可改。
73. **上传页策略提示必须动态读取** — `client/pages/upload.vue` 管理员上传使用“目标图库 / 用户图库 / 目标相册”三联 `TaotuSelect`，默认“我的图库”；“全部”显示全部用户相册，“用户图库”先选用户再选该用户相册。普通用户只显示自己的目标相册。后端 `/api/upload` 必须校验普通用户只能上传到自己相册，管理员才可上传到任意相册。新标签输入按 Enter 必须加入 `uploadConfig.newTags` 暂存；上传结果必须显示每张图实际 `storage_strategy_name`。上传页格式/单图大小提示不能写死，必须读 `/api/upload/policy`，由当前登录用户组动态拼接；用户组 `max_file_size_mb=0` 和 `max_concurrent_uploads=0` 都表示不限制。单图大小只归用户组管，用户管理弹窗禁止恢复用户级单图大小覆盖项，不能回退到旧全局默认 50/100 或用户旧字段造成“最大 100MB/张”这类幽灵限制。
74. **布尔状态统一图标化** — 后台和仪表盘列表里的“是/否”状态统一使用 `BoolStatusIcon`（绿色圆勾、粉色圆叉），不要回退为文字胶囊或散落字符。布尔列标题优先使用“公共”“可组合”“递归”这类短标题，图标对齐应以表头文字宽度为基准居中，而不是按整列宽度强制居中。
75. **加载态统一 SVG 动画** — 页面加载中的控制类占位必须使用 `TaotuIcon name="loading"` 或同体系 SVG，不要新增 GIF/WebP 占位文件；`loading/progress` 图标在 `TaotuIcon` 内统一加旋转动画，详情页加载态也必须复用该 SVG 规则。
...
| 表 | 用途 |
|---|------|
| `images` | 图片信息、路径、哈希路径、公共标记、上传者、存储策略、NSFW 状态 |
| `albums` | 相册信息、公共标记、创建者、封面、主页隐藏 |
...
| `storage_strategies` | 本地/COS/OSS/FTP/SFTP 存储策略配置 |
| `storage_strategy_groups` | 存储策略与用户组的多对多关联 |
...
> v0.3.2 是 0.3.1 正式版后的正式功能收口：大框架和业务接口保持稳定，在后端 ES Module、无感会话、权限隔离、私有媒体门禁、页面 Q 弹转场、人工标签预览、WebDAV 启动兼容、Boxicons 图标体系与精致下拉/弹窗 UI 规范基础上，继续追加相册级主页可见性、用户组、存储策略、严格 NSFW 审核、自定义路径删减开关和上传页管理员目标图库选择。
...
- 页面切换 Q 弹转场已分层并增强力度：图片详情固定模糊背景先渲染，背景图完成或缓存确认后立即弹出内容层；后台子页保留页面根的粉蓝背景和伪元素静态，只对页面根下一级内容做 Q 弹动画；详情加载态统一使用可旋转 SVG loading 图标，不再保留 GIF 占位资源。
...
- 用户组与存储策略是 `0.3.2` 管理员功能：`user_groups` 负责上传限制、命名规则、图片处理参数和 NSFW 审核配置，`storage_strategies` + `storage_strategy_groups` 负责存储策略与多个用户组的共享绑定。默认用户组与默认本地策略由迁移自动创建；当前只有本地策略真实参与上传，第三方 COS/OSS/FTP/SFTP 先提供配置入口并在上传时显式拒绝。若同一用户组绑定多个策略，当前上传不会多写多份，也没有容量兜底轮换，只按非默认优先、ID 较大优先选择一个策略。
- NSFW 审核状态写入 `images.nsfw_status/nsfw_detail`；上传时必须同步审核出明确安全/不安全结果，审核服务不可用会拒绝上传并回滚入库文件，后台定时补扫未审核或重排队历史图片。非管理员公开展示、随机图和媒体直链必须过滤 `nsfw_status=true`，管理视图可以保留记录用于删除或处置。`nsfw.js` 兼容官方 predictions 与 `arnidan/nsfw-api` 扁平小写返回，配置探针为 `299x299` JPEG。
...
- 布尔状态、NSFW 健康内容和加载状态已统一：列表“是/否”使用 `BoolStatusIcon` 的绿色圆勾/粉色圆叉，并按表头文字宽度对齐；NSFW 未开启/未检测使用 filled `help-circle`，颜色 `#ffb226`；加载中使用 `TaotuIcon name="loading"` 的旋转 SVG，不要恢复文字“是/否”、文字“无”、CSS 伪图标、静态 loading 或 GIF 占位。
...
- SMTP、公开域名、备案号、注册开关、网站隐私模式、背景、WebDAV、默认展示、配额等全部写入 `site_config`；隐私模式关闭时保持访客公共访问，开启时必须由 `server/middleware/privacyMode.js` 在公开 API 和媒体直链层强制拦截无登录/无 Token 请求。
...
- 发布路线：当前为 `0.3.2` 正式版；本轮完成相册主页隐藏、用户组上传规则、存储策略管理、严格 NSFW 审核、自定义路径删减开关和上传页管理员目标图库选择。下个预备版本 `0.3.3-pre` 将优先完善点赞、收藏等图床互动能力，稳定后发布 `0.3.3`；移动端重新适配、主副标题拆分和面板内容收纳进入后续 `0.4.0` / `0.4.x`，稳定后再进入 `1.0.0`。
