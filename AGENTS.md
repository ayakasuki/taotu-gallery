...
11. **互斥标签字段是字符串** — `tags.mutually_exclusive_with` 必须是 `varchar`，迁移 `20240115000001_change_mutual_exclusive_to_string.js` 必须执行。支持多个 ID、`.`/`,`/中文标点/空白分隔，用户标签 ID 允许 `u13`。
12. **互斥标签按组闭包保存** — 例如标签 6 设置 `3,4` 后，3/4/6 三者必须互相写入互斥关系。清空某标签互斥关系时，前端需同步从其它标签互斥列表移除该标签。
13. **标签筛选 API 必须校验互斥参数** — 对外 `/api/images*`、`/api/embed/*` 和内部图库接口使用 `server/utils/tagConflict.js`，非法组合返回 `400 JSON`，不要只返回空结果。
14. **用户私有标签不可误删** — 管理员维护图片平台标签时不能覆盖或删除图片所属用户已有私有标签；如需维护用户私有标签，走独立的用户私有标签接口。
...
# 后端
npm install
npx knex migrate:latest     # 数据库迁移
npm run dev                  # 启动开发服务器（端口 7067）
