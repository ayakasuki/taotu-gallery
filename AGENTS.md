...
72. **布尔状态统一图标化** — 后台和仪表盘列表里的“是/否”状态统一使用 `BoolStatusIcon`（绿色圆勾、粉色圆叉），不要回退为文字胶囊或散落字符。图标对齐应以表头文字宽度为基准居中，而不是按整列宽度强制居中，避免“是否公共/是否可组合/递归”等列和表头错位。
73. **加载态统一 SVG 动画** — 页面加载中的控制类占位必须使用 `TaotuIcon name="loading"` 或同体系 SVG，不要新增 GIF/WebP 占位文件；`loading/progress` 图标在 `TaotuIcon` 内统一加旋转动画，详情页加载态也必须复用该 SVG 规则。
...
- 布尔状态和加载状态已统一：列表“是/否”使用 `BoolStatusIcon` 的绿色圆勾/粉色圆叉，并按表头文字宽度对齐；加载中使用 `TaotuIcon name="loading"` 的旋转 SVG，不要恢复文字“是/否”、CSS 伪图标、静态 loading 或 GIF 占位。
