...
38. **前台主导航保持简洁** — 默认布局主导航按钮只保留文字，不再显示栏目图标；登录、通知、用户胶囊等独立控件可以保留自己的图标。
39. **图库首页透明导航只在顶部生效** — 仅 `/` 页面滚动到顶部时隐藏导航背景/模糊/分隔线并让主导航文字变白；离开顶部或进入其它页面必须恢复常规玻璃导航。
40. **导航左右端跟随页面容器** — `brand-link` 和通知/用户区应通过默认布局内的 `nav-frame` 对齐图库、相册、API 文档、上传、仪表盘等页面主体左右边界，宽屏不要贴浏览器边缘。
41. **移动端重适配后置** — 当前 `0.3.1` 系列以桌面端主要流程稳定为准；移动端主副标题拆分、面板收纳和窄屏重排属于后续 `0.4.x` 迭代，再稳定进入 `1.0.0`。
