...
## 当前实现状态（v0.3.1-pre-fix3）

> v0.3.1-pre-fix3 是 0.3.1 正式版发布前的第三个本地预发布修复点：大框架保持稳定，当前重点收口图片派生缩略图、路径导入清理、相册/图片批量操作、嵌入代码尺寸同步和统一删除确认；部分图标仍允许使用占位资源，等待最终图标替换。
...
- 发布路线：当前为 `0.3.1-pre-fix3`；图标替换后发布 `0.3.1` 正式版；移动端重新适配、主副标题拆分和面板内容收纳进入后续 `0.4.0` / `0.4.x`，稳定后再进入 `1.0.0`。
